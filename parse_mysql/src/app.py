...
class app():
    ...
    def abrir_xml(self, archivo_xml):
        ...
        with open('datos_corregir.json', 'r') as archivo:
...
def envia_mensajes_error(error, data, class_sqs):
    error = str(error)
    if(os.environ['LAMBDA_PRINCIPAL'] == '1_7'):
        cola_principal = 'parseo_listoXml1_7'
    elif(os.environ['LAMBDA_PRINCIPAL'] == '7_n'):
        cola_principal = 'parseo_listoXml7_n'
    else:
        cola_principal = 'listoXml'
    if(not error or errores_base(error)):
        if(class_sqs.envia_msg(json.dumps(data), cola_principal) != True):
            print("Error al enviar mensaje base", json.dumps(data))
    else:
        print("ERROR", data, error)
        data["msg"] = 'Error no controlado'
        data["error"] = limpia_cadena_errores(error)
        if(class_sqs.envia_msg(json.dumps(data), 'parseoError') != True):
...
def registra_cfdi_si_o_no(datos_parseo, conexion, data, class_sqs, class_aws, archivo):
    ...
    if(result[0] == True):
        ...
        if siparseo == '' or siparseo == None:
            ...
            print("ya esta registrado", datos_parseo)
    ...
    with funcionesMySQL() as class_conexion:
        for record in event['Records']:
            ...
            if class_conexion is None or class_conexion is  False:
                class_sqs.envia_msg(json.dumps(data), 'parseo_detalle_conexiones')
                continue
            rfc = data['rfc']
            ...
            if(os.environ['LAMBDA_PRINCIPAL'] == '1_7'):
                ...
                cola_principal = 'listoXml'
            #VALIDAMOS QUE ESTE MENSAJE NO SEA CONSUMIDO POR LA LAMBDA MAS DE 3 VECES
            cuantas_veces = 0
            try:
                cuantas_veces = data['cuantas_veces']
            except Exception:
              data['cuantas_veces'] = 0
            if(cuantas_veces == 3):
                class_sqs.envia_msg(json.dumps(data), 'parseo_detalle_conexiones')
                continue
            #print(data)
            ...
            try:
                ...
                #print(descargo_archivo)
                data['cuantas_veces'] = int(cuantas_veces) + 1
                if(descargo_archivo == True):
                    ...
                    if(resp_parseo[0] == True):
                        ...
                        if(datos_parseo['uuid'] !='00000000-0000-0000-0000-000000000000'):
                            ...
                            #print("resul_consulta", resul_consulta)
                            if(resul_consulta):
                                tablas = []
                                ...
                                if(registro_base_tabla_principal == True and registro_base_tabla_detalladas == True):
                                    ...
                                    if(class_conexion.mysql_update(datos, 'compulsa', where_update) == True):
                                        ...
                                        if('Duplicate entry' in mensaje_error):
                                            class_sqs.envia_msg(json.dumps(data), cola_principal)
                                        else:
                        ...
                        if(class_sqs.envia_msg(json.dumps(msg), 'parseoError') != True):
